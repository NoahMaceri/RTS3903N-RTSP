...
The hardware has no RTC, the stock busybox lacks `ntpd`/`ntpdate`/`rdate`, and the wget build is too stripped to fetch the HTTP `Date:` header. Fix: ship our own tiny `sntp` binary (`src/sntp/sntp.cpp`, ~60 lines C). One UDP/123 round-trip, `settimeofday()`, exit. `config.sh` runs it backgrounded at the end of boot so a network hiccup doesn't block daemon startup; result lands in `boot.log`.
